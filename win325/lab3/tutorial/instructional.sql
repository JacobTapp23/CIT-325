...
DECLARE
  /* Declare a collection of strings. */
  TYPE list IS TABLE OF VARCHAR2(11);
  /* Declare a variable of the string collection. */
  lv_strings  LIST := list('10-JAN-2001','29-FEB-2003','28-FEB-15','28-FEB-2016','31-JUN-2007');
  lv_date    DATE;
BEGIN
  /* Check and assign valid date. */
  FOR i IN 1..lv_strings.LAST LOOP
    IF verify_date(lv_strings(i)) IS NOT NULL THEN
      ...
      dbms_output.put_line('Does match date:     ['||lv_strings(i)||']['||lv_date||']');
    ELSE
      lv_date := verify_date(lv_strings(i));
      dbms_output.put_line('Does not match date: ['||lv_strings(i)||']['||lv_date||']');
    END IF;
  END LOOP;
END;
/

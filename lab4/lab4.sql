...
BEGIN
  ...
  */
  ...
  --   IF j > 1 THEN
  --    dbms_output.put_line(' '||lv_mime(j));
  --   ELSIF i = j THEN
  --   dbms_output.put_line(' A[lv_mime('||j||')]');
  --   ELSE
  --   dbms_output.put_line(' and a[lv_mime('||j||')]');
  --   END IF;
  ...
  /* Couple the behavior of two lists in two loops. */
  FOR i IN 1..lv_list.LAST LOOP
  dbms_output.put_line('on the '||lv_list(i)||' day of Christmas');
